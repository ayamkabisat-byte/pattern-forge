...
type PendingPattern = { target: PatternTarget; asset: PatternAsset }
...
function uid() { return crypto.randomUUID().replaceAll('-', '').slice(0, 18) }
...
const deepClone = <T,>(value?: T) => value ? JSON.parse(JSON.stringify(value)) as T : undefined
function notifyLibrary() { window.dispatchEvent(new CustomEvent(LIBRARY_EVENT)) }
export function libraryEventName() { return LIBRARY_EVENT }
...
function writeLibrary(items: PatternAsset[]) { localStorage.setItem(LIBRARY_KEY, JSON.stringify(items)); notifyLibrary() }
...
export function deletePatternAsset(id: string) { writeLibrary(loadPatternLibrary().filter((item) => item.id !== id)) }
...
export function encodeGridCells(cells: Uint8Array) {
  ...
  for (let offset = 0; offset < cells.length; offset += chunk) binary += String.fromCharCode(...cells.subarray(offset, Math.min(cells.length, offset + chunk)))
  return btoa(binary)
}

export function decodeGridCells(data: string, expectedLength: number) {
  try { const binary = atob(data); const cells = new Uint8Array(expectedLength); const limit = Math.min(binary.length, expectedLength); for (let index = 0; index < limit; index++) cells[index] = binary.charCodeAt(index); if (binary.length < expectedLength) cells.fill(255, binary.length); return cells } catch { return new Uint8Array(expectedLength).fill(255) }
}

function gridPaths(grid: GridPatternData) {
  const cells = decodeGridCells(grid.cellsBase64, grid.width * grid.height); const transparent = grid.transparentValue ?? 255; const paths = new Map<number, string[]>()
  for (let y = 0; y < grid.height; y++) { let x = 0; while (x < grid.width) { const value = cells[y * grid.width + x]; const start = x; while (x + 1 < grid.width && cells[y * grid.width + x + 1] === value) x++; const length = x - start + 1; if (value !== transparent && value < grid.palette.length) { const list = paths.get(value) ?? []; list.push(`M${start} ${y}h${length}v1H${start}z`); paths.set(value, list) } x++ } }
...
function gridSvgDocumentSize(asset: PatternAsset, grid: GridPatternData) {
  const configured = Number(asset.meta?.exportLongSide); if (!Number.isFinite(configured) || configured <= 0) return { width: grid.width, height: grid.height }
  const longSide = Math.max(64, Math.min(20000, Math.round(configured))); const scale = longSide / (Math.max(grid.width, grid.height) || 1)
  return { width: Math.max(1, Math.round(grid.width * scale)), height: Math.max(1, Math.round(grid.height * scale)) }
}

export function patternAssetToSvg(asset: PatternAsset) {
  if (asset.grid) { const { width, height } = asset.grid; const documentSize = gridSvgDocumentSize(asset, asset.grid); const seamless = asset.meta?.cropped ? 'false' : 'true'; return `<svg xmlns="http://www.w3.org/2000/svg" width="${documentSize.width}" height="${documentSize.height}" viewBox="0 0 ${width} ${height}" shape-rendering="crispEdges" data-patternforge-exact-bounds="true" data-patternforge-seamless="${seamless}">${gridPaths(asset.grid)}</svg>` }
...
export function peekPendingPattern(target?: PatternTarget) { const pending = safeParse<PendingPattern | null>(localStorage.getItem(PENDING_KEY), null); if (!pending) return null; if (target && pending.target !== target) return null; return pending }
export function consumePendingPattern(target: PatternTarget) { const pending = peekPendingPattern(target); if (!pending) return null; localStorage.removeItem(PENDING_KEY); return pending.asset }
