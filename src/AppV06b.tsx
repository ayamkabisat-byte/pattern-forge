...
import { buildSvgFromGeometry } from './engine/tileSvg'
...
export default function App() {
  ...
  const [message, setMessage] = useState('Upload SVG motifs. Final Canvas shows the actual export composition.')
  ...
  function refill(kind: 'sequential' | 'random') {
    const g = builderGeometry(assets, settings, builderTile)
    setPlacements(kind === 'sequential' ? fillSequential(assets, g) : fillRandom(assets, g, settings.seed))
  ...
  function masterSvg() {
    if (workspaceMode === 'builder') {
      return buildSvgFromGeometry(assets, instances, settings, geometry)
    }
    return buildSvg(mode, assets, instances, settings)
  }
  ...
  function exportSvg() {
    ...
    downloadBlob(
      new Blob([masterSvg()], { type: 'image/svg+xml;charset=utf-8' }),
      `patternforge-${label}-tile-${Math.round(geometry.tileWidth)}x${Math.round(geometry.tileHeight)}.svg`,
    )
  }
  ...
  async function exportPng() {
    ...
    try {
      ...
      setMessage(`Exported ${exportSettings.width} × ${exportSettings.height} PNG.`)
    } catch (error) {
      ...
      <main className="workspace">
            ...
            <div className="asset-list">
              {assets.map((asset, index) => (
                <div className={`asset-card ${workspaceMode === 'builder' && builderTool === 'paint' && activeAssetId === asset.id ? 'active-asset' : ''}`} key={asset.id} onClick={() => { if (workspaceMode === 'builder') { setBuilderTool('paint'); setActiveAssetId(asset.id) } }}>
                  ...
                  <div><b>{asset.name}</b><span>{workspaceMode === 'builder' ? 'Click to use Paint' : `Motif ${index + 1}`}</span></div>
            ...
            <>
              ...
              <section>
                ...
                <div className="builder-actions">
                  <button onClick={() => refill('sequential')} disabled={!assets.length}>Fill Sequential</button>
                  <button onClick={() => refill('random')} disabled={!assets.length}>Fill Random</button>
        ...
        <section className="stage-wrap">
          <div className="stage-toolbar">
            <div>
              <b>{workspaceMode === 'builder' ? (builderView === 'edit' ? 'Master Tile Editor' : builderView === 'canvas' ? 'Final Export Canvas' : 'Seamless Proof') : 'Live Seamless Proof'}</b>
              <span>{workspaceMode === 'builder' && builderView === 'canvas' ? `${exportSettings.width} × ${exportSettings.height} · ${canvasModeLabel(exportSettings.canvasMode ?? 'full-bleed')}` : `Master tile ${Math.round(geometry.tileWidth)} × ${Math.round(geometry.tileHeight)}`}</span>
            </div>
            {workspaceMode === 'builder' && (
          ...
          <div className="stage canvas-stage">
            {!assets.length ? (
              <div className="empty-stage"><div className="empty-mark">✦</div><h1>Start with SVG motifs</h1><p>Choose a final canvas, build the master tile, and preview the exact output before export.</p></div>
          ...
          <div className="statusbar">
            <span>{message}</span>
            <span>{noCut ? `Whole tiles ${canvasLayout.columns} × ${canvasLayout.rows}` : `Layout ${canvasLayout.columns} × ${canvasLayout.rows}`} · drawn tile {Math.round(canvasLayout.tileWidth)} × {Math.round(canvasLayout.tileHeight)}</span>
          </div>
        </section>
        ...
        <aside className="sidebar right-panel">
          {workspaceMode === 'builder' && (
            <section>
              ...
              <SelectRow label="Sizing" value={builderTile.mode} onChange={(v) => setBuilderTile((current) => ({ ...current, mode: v as BuilderTileSettings['mode'] }))}>
                <option value="grid">Auto from Grid</option>
                <option value="custom">Custom Master Tile</option>
              </SelectRow>
              <div className="master-presets">{[500, 1000, 2000, 3000].map((size) => <button key={size} onClick={() => setBuilderTile({ mode: 'custom', width: size, height: size })}>{size}</button>)}</div>
              ...
              <button className="full master-match" onClick={() => setBuilderTile({ mode: 'custom', width: exportSettings.width, height: exportSettings.height })}>Match Master Tile to Canvas</button>
          ...
          <section>
            <h2>Grid & Spacing</h2>
            <div className="grid-presets">{[2, 4, 6, 8].map((size) => <button key={size} onClick={() => setSettings((current) => ({ ...current, columns: size, rows: size }))}>{size}×{size}</button>)}</div>
            <div className="two-col compact">
          ...
          {workspaceMode === 'builder' ? (
            <section>
              ...
              ) : <p className="builder-help">Select a cell in Edit Tile mode.</p>}
            </section>
          ) : (
            <section>
              <h2>Motif</h2>
          ...
          <section className="canvas-panel">
            ...
            <div className={`canvas-summary ${noCut ? 'safe' : ''}`}>
              ...
              <span>{canvasLayout.columns} × {canvasLayout.rows} tiles · draw size {Math.round(canvasLayout.tileWidth)} × {Math.round(canvasLayout.tileHeight)}</span>
              {noCut && <small>No partial tile. Remaining space is centered background margin.</small>}
            </div>
          ...
          <section className="batik-note">
            ...
            <p><b>Final Canvas</b> mirrors the exact export aspect ratio. For 1920×1080 you can see the final composition before saving.</p>
            <p><b>Fit Full Tiles</b> uses only complete tiles; <b>Full Bleed</b> fills every pixel but can crop motifs at the edge.</p>
          </section>
        </aside>
      </main>
    </div>
